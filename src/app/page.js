...
export default function KeepClone() {
  ...
  // Filtrer et trier les notes (avec debug)
  ...
  // Trier par orderIndex puis par date de création (avec debug)
  ...
  const pinnedNotes = sortedNotes.filter(note => note && note.pinned);
  const unpinnedNotes = sortedNotes.filter(note => note && !note.pinned);
